...
window.onload = function (){
    BlockElasticScroll();
    /*if(IsUnderIpad()){
        window.parent.bind(this, 'viewappearing', onViewAppearingHandler);
        window.parent.bind(this, 'viewappeared', onViewAppearedHandler);
        window.parent.bind(this, 'viewdisappeared', onViewDisappearedHandler);
    }
    else{*/
        PreInitialize();
        startpage();
...
function GetDisparitionClass(_Popup){
    ...
    if(undefined != _Popup && _Popup.id.indexOf("reference")>0 && undefined != savedPopupClasses[_Popup.id]){
